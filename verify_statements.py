...
def embed_texts(client: OpenAI, model: str, inputs: List[str]) -> np.ndarray:
	"""Embed texts with token-aware batching to avoid exceeding OpenAI's token limits."""
	# Initialize tokenizer for the embedding model
	try:
		# Use cl100k_base encoding which is used by text-embedding-3-large
		tokenizer = tiktoken.get_encoding("cl100k_base")
	except Exception as e:
		print(f"Warning: Could not initialize tokenizer: {e}. Falling back to character-based batching.")
		tokenizer = None
	...
	for text in inputs:
		...
		if current_batch_tokens + text_tokens > MAX_TOKENS_PER_REQUEST and current_batch:
